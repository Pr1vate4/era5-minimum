...
```bash
docker compose config
docker compose --profile tools run --rm --no-deps tools pytest -q
```

GitHub Actions использует Python 3.12, устанавливает `.[dev]`, запускает
pytest, offline download dry-runs и synthetic smoke.

На момент последнего README audit полный pytest collection имеет известные
blockers:

- пустой пакет `era5_minimum.metrics` перекрывает модуль `metrics.py`;
- container test `test_prepare_globe_assets.py` не импортирует `scripts` как
  package.

Bundled artifact validation и frontend production build проходят отдельно.
README намеренно не публикует coverage percentage или badge `passing`, пока
полный suite не восстановлен.

В репозитории нет ESLint, Ruff, mypy, Prettier и frontend unit/e2e test runner;
не следует подразумевать их наличие.

## Типичный сценарий исследователя

1. Зафиксировать data contract, каналы, units, timestamps и checksums.
2. Подготовить leakage-safe train/validation/test manifest с temporal gap.
3. Рассчитать normalization statistics только по training subset.
4. Запустить PCA baseline, затем controlled ConvAE/CRA5 experiment на тех же
   split и метриках.
5. Создать checkpoint и standalone bitstream.
6. Выполнить decode без доступа к исходному tensor.
7. Посчитать physical per-channel metrics и serialized compression ratio.
8. Сохранить config, seed, commit, runtime и artifact metadata.
9. Подготовить original/reconstruction/error assets.
10. Проверить глобальные и локальные ошибки через API, dashboard и globe.
11. Повторить эксперимент на nested train sizes и найти область насыщения
    качества.

## Для чего можно использовать МетеоКод

- разработка и проверка atmospheric compression pipelines;
- исследование зависимости reconstruction quality от объёма train data;
- сравнение PCA, convolutional и transfer-learning codec paths;
- анализ пространственного распределения ошибок по каналам и уровням;
- проверка реального bitstream accounting и exact symbol roundtrip;
- визуальная валидация historical ERA5 fields и model artifacts;
- демонстрация воспроизводимых экспериментов через API/dashboard.

## Ограничения и следующие этапы

### Текущие ограничения

- В Git нет полного research dataset, финальных checkpoints и подтверждённых
  32×/64× результатов на real ERA5.
- Bundled dashboard metrics демонстрационные.
- Real-data ConvAE training и live API reconstruction/error не подключены.
- Scientific 0.5° conservative remapping не реализован.
- CRA5 требует внешний checkpoint и изолированный upstream runtime; текущий
  adapter trainer использует synthetic data.
- Full pytest suite имеет описанные выше import blockers.
- Canonical Docker image CPU-only.
- Актуальный screenshot интерфейса не хранится в репозитории.

### Подтверждённые направления

- согласовать и подготовить финальный ERA5 train range;
- завершить conservative remap с проверяемыми weights;
- восстановить зелёный Python suite;
- подключить real-data training к codec CLI;
- сравнить nested train sizes при фиксированных validation/test;
- передать reconstruction/error artifacts в live API;
- добавить reviewed dashboard screenshot без demo ambiguity.
...
| Документ | Назначение |
| --- | --- |
| [DATA_CONTRACT.md](docs/DATA_CONTRACT.md) | Строгий 8-channel raw ERA5 contract |
| [ERA5_28CH_DATA_PIPELINE.md](docs/ERA5_28CH_DATA_PIPELINE.md) | WeatherBench2/Zarr 28-channel pipeline |
| [ERA5_28CH_DEMO_RUNBOOK.md](docs/ERA5_28CH_DEMO_RUNBOOK.md) | Локальный real-data integration demo |
| [ARTIFACT_API_CONTRACT.md](docs/ARTIFACT_API_CONTRACT.md) | JSON artifact и REST contract |
| [GLOBE.md](docs/GLOBE.md) | Globe manifest, assets и coordinate sampling |
| [DOCKER.md](docs/DOCKER.md) | CPU containers, mounts и runtime profiles |
| [MONITORING.md](docs/MONITORING.md) | Prometheus/Grafana stack и metric contract |
| [DECISIONS.md](docs/DECISIONS.md) | Принятые исследовательские и инженерные решения |
| [DEVELOPMENT.md](docs/DEVELOPMENT.md) | Development workflows |
| [SECURITY.md](SECURITY.md) | Credentials, data и artifact security |

## Contributing

Правила веток, pull requests, Definition of Done и scientific review находятся
в [CONTRIBUTING.md](CONTRIBUTING.md). Изменения data split, normalization,
channel order, units и metric definitions требуют отдельного обоснования,
обновления tests и записи в `docs/DECISIONS.md`.

## Лицензия

В репозитории пока отсутствует отдельный файл лицензии. До его появления
нельзя предполагать разрешение на использование, распространение или создание
производных работ только на основании публичной доступности исходного кода.
